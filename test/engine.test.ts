...
async function makeEngine(root: string, provider: RevisionProvider, responses: ConstructorParameters<typeof MockAgentRunner>[0], configure?: (config: WorkflowConfig) => void) {
    ...
    try {
      ...
      try {
        ...
        for (const row of rows) {
          ...
          const text = serializeHandoff({ ...handoff, memory: [{ content: "noise".repeat(2000) }], changedFiles: Array.from({ length: 100 }, (_, i) => `long/path/${i}.ts`) }, 2000);
