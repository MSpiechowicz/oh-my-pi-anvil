...
describe("workflow configuration", () => {
  test("loads nested YAML lists without changing the fixed workflow", async () => {
    const root = await mkdtemp("/tmp/anvil-config-"); await mkdir(path.join(root, ".omp"), { recursive: true }); await writeFile(path.join(root, ".omp", "orchestrator.yml"), "version: 1\nworkflow:\n  name: secure-code-change\nchecks:\n  - id: typecheck\n    command: [bun, run, typecheck]\n    required: true\n    timeoutMs: 1000\n"); const config = await loadConfig(root); expect(config.checks).toHaveLength(1); expect(config.checks[0].id).toBe("typecheck"); expect(config.checks[0].command).toEqual(["bun", "run", "typecheck"]); await rm(root, { recursive: true, force: true });
  });
