...
function rejectUnknownKeys(value2, allowed, label) {
  if (!value2 || typeof value2 !== "object" || Array.isArray(value2)) throw new AnvilError("CONFIG_INVALID", `${label} must be an object`);
...
function normalizeLimit(value2, label, integer = false) {
  if (value2 === void 0 || value2 === null) return null;
  if (typeof value2 !== "number" || !Number.isFinite(value2) || value2 <= 0 || integer && !Number.isInteger(value2)) throw new AnvilError("CONFIG_INVALID", `${label} must be a positive finite ${integer ? "integer" : "number"} or null`);
  return value2;
}
function validateConfig(config) {
  ...
  for (const role of WORKFLOW_ROLE_ORDER) {
    const policy = config.budgets.perRole[role] === void 0 ? config.budgets.perRole[role] = {} : config.budgets.perRole[role];
  ...
  for (const check of config.checks ?? []) {
    ...
    if (check.env != null && (typeof check.env !== "object" || Array.isArray(check.env) || Object.values(check.env).some((value2) => typeof value2 !== "string"))) throw new AnvilError("CONFIG_INVALID", `Check ${check.id} env must contain string values or be null`);
    ids.add(check.id);
  }
...
var GitRevisionProvider = class {
  ...
  async workspaceFiles(untracked) {
    ...
    const files = [];
    const untrackedHashes = new Map(untracked.map((relative) => [
      relative,
      void 0
    ]));
    const paths = /* @__PURE__ */ new Set([
    ...
    const worker = async () => {
      try {
        while (!failed) {
          ...
          if (entry) {
            const object = createHash2(entry.object.length === 64 ? "sha256" : "sha1").update(`blob ${file.content.length}\0`).update(file.content).digest("hex");
            if (object !== entry.object || file.mode !== entry.mode) matchesIndex = false;
          } else {
            matchesIndex = false;
          }
          if (untrackedHashes.has(relative)) untrackedHashes.set(relative, {
            path: relative,
...
async function runProcess(command, input) {
  ...
  try {
    ...
    const timeout = input.timeoutMs == null ? void 0 : setTimeout(() => {
      timedOut = true;
      child.kill("SIGKILL");
    }, input.timeoutMs);
    let exitCode;
...
function nativeUsageBus(context, request) {
  ...
  const flush = () => {
    ...
    pending = void 0;
    try {
      const result = invoke(manager, "appendModelUsage", [
        entry,
        target
      ]);
  ...
  const thinkingFor = (id, provider, model) => {
    const resolved = id ? metadata.get(id) : void 0;
  ...
  return {
    async settle() {
      ...
      if (failure2) throw failure2.error;
    },
    emit(name, payload) {
      const record2 = asRecord(payload);
      if (name === "task:subagent:progress") {
        const progress = asRecord(record2?.progress);
        const id2 = stringValue(progress?.id);
        if (!id2) return;
        ...
        if (pending?.id === id2) {
          pending.entry.thinkingLevel = thinkingFor(id2, pending.entry.provider, pending.entry.model);
        }
        return;
      }
      if (name === "task:subagent:lifecycle") {
        flush();
        const id2 = stringValue(record2?.id);
        if (id2) metadata.delete(id2);
        return;
      }
      if (name !== "task:subagent:event") return;
      flush();
      const id = stringValue(record2?.id);
      const event = asRecord(record2?.event);
      ...
      const current = {
        ...
        entry: {
          ...
          model,
          usage,
          thinkingLevel: thinkingFor(id, provider, model)
        }
      };
      pending = current;
      queueMicrotask(() => {
...
function nativeExecutorOptions(context, request, agent, settings, eventBus) {
  ...
  const options = {
    ...
    thinkingLevel: request.thinkingLevel ?? void 0,
    ...request.effort != null ? {
...
function nativeTaskSession(context, request, settings, eventBus) {
  const contextRecord = asRecord(context);
  const session = {
    cwd: request.cwd,
    hasUI: false,
...
async function executeNativeSubprocess(context, host, request) {
  ...
  try {
    const raw = await invoke(host, "runSubprocess", [
      nativeExecutorOptions(context, request, agent, settings, eventBus)
    ]);
...
var BudgetManager = class {
  ...
  assertMayContinue(run) {
    ...
    if (limits.maxTotalTokens != null && run.usedTokens >= limits.maxTotalTokens) throw new AnvilError("BUDGET_EXHAUSTED", `Total token budget exhausted: ${run.usedTokens} consumed / ${limits.maxTotalTokens} limit. This is the executor-reported aggregate, including cached tokens when the host includes them, not monetary cost (input + output fallback when no total is reported). Recorded run cache components: read ${run.usedCacheReadTokens ?? "unknown"}, write ${run.usedCacheWriteTokens ?? "unknown"}; reporting may be incomplete. Limits are checked between stages, not by interrupting an in-flight child. Review budgets.maxTotalTokens, then raise or remove that cap if appropriate and resume with /anvil resume ${run.id}.`);
    if (limits.maxTotalRequests != null && run.usedRequests >= limits.maxTotalRequests) throw new AnvilError("BUDGET_EXHAUSTED", "Total request budget exhausted");
    if (limits.maxTransitions != null && run.transitionCount >= limits.maxTransitions) throw new AnvilError("BUDGET_EXHAUSTED", "Workflow transition budget exhausted");
    if (limits.maxWallClockMs != null && Date.now() - Date.parse(run.startedAt ?? run.createdAt) >= limits.maxWallClockMs) throw new AnvilError("BUDGET_EXHAUSTED", "Workflow wall-clock budget exhausted");
  }
  assertRoleMayRun(_run, role, attempts, roleTokens = 0, roleRequests = 0) {
    const rolePolicy = this.config.budgets.perRole[role];
    if (rolePolicy?.maxAttempts != null && attempts >= rolePolicy.maxAttempts) throw new AnvilError("MAX_ATTEMPTS_EXCEEDED", `${ROLE_LABELS[role]} attempt budget exhausted`);
    if (rolePolicy?.maxTokens != null && roleTokens >= rolePolicy.maxTokens) throw new AnvilError("BUDGET_EXHAUSTED", `${ROLE_LABELS[role]} token budget exhausted: ${roleTokens} consumed / ${rolePolicy.maxTokens} limit for this role. This is the role's executor-reported aggregate, including cached tokens when the host includes them, not monetary cost (input + output fallback when no total is reported). Limits are checked between stages, not by interrupting an in-flight child. Review budgets.perRole.${role}.maxTokens, then raise or remove that cap if appropriate and resume with /anvil resume ${_run.id}.`);
    if (rolePolicy?.maxRequests != null && roleRequests >= rolePolicy.maxRequests) throw new AnvilError("BUDGET_EXHAUSTED", `${ROLE_LABELS[role]} request budget exhausted`);
  }
};
...
var SMITH_TASK_INSTRUCTIONS = " Return smithTasks to explicitly decompose Smith work: each task needs a unique id, objective, dependsOn task IDs, ownedFiles workspace-relative paths, nonempty acceptanceCriteria, and findingIds. Assess the full plan for independent work packages and use separate tasks with disjoint ownership where safe; use one explicit task for coupled work. Empty or uncertain ownership is exclusive; only explicit disjoint ownership permits concurrency. Do not spawn workers yourself.";
var GATE_TASK_INSTRUCTIONS = " You may optionally propose a Smith repair dispatch." + SMITH_TASK_INSTRUCTIONS + " For findings introduced in this output, use their zero-based findings array indices as decimal strings in smithTasks.findingIds; existing supplied open finding IDs are also allowed. Tasks must cover every resulting open finding, including findings from other gates.";
var WorkflowEngine = class {
  ...
  async start(input) {
    ...
    let run = this.runs.create({
      ...
      maxTotalTokens: this.deps.config.budgets.maxTotalTokens ?? void 0,
      maxTotalRequests: this.deps.config.budgets.maxTotalRequests ?? void 0,
        ...
        try {
          ...
          switch (run.currentState) {
            ...
            case "CHECKS":
              ...
              break;
            case "SECURITY":
              run = await this.executeSecurity(run, signal, reviewEvidence);
              break;
            case "REVIEW":
              run = await this.executeReview(run, signal, reviewEvidence);
              break;
            default:
  ...
  async prepareSmithDispatch(run, signal) {
    ...
    if (!tasks) {
      ...
      this.budget.assertRoleMayRun(run, "planner", attempts.length, attempts.reduce((sum, attempt2) => sum + attempt2.tokens, 0), attempts.reduce((sum, attempt2) => sum + attempt2.requests, 0));
      const shared = await this.sharedContext(run);
      if (recovery) {
  ...
  async executeSecurity(run, signal, reviewEvidence) {
    const before = await this.deps.revisions.current();
    ...
    this.budget.assertRoleMayRun(run, "security", securityAttempts.length, securityAttempts.reduce((total, attempt2) => total + attempt2.tokens, 0), securityAttempts.reduce((total, attempt2) => total + attempt2.requests, 0));
    const prepared = await this.prepareGateHandoff(run, "security", before.head, reviewEvidence);
  ...
  async report(run, kind) {
    ...
    for (const role of [
      ...
      const enabled = role === "scout" ? this.deps.config.scouting.enabled : this.deps.config.memory.enabled && this.deps.config.memory.retainOnSuccess && this.deps.config.memory.archivist;
      if (!enabled) continue;
      const attempt = attempts.find((item) => item.role === role);
      if (attempt) {
        advisory[role] = attempt.status === "running" ? "running" : attempt.status === "completed" && attempt.verdict === "advisory" ? "completed" : "failed";
      } else {
        const phasePassed = role === "scout" && (run.currentState !== "INIT" && run.currentState !== "PLAN" || attempts.some((item) => item.role === "planner"));
  ...
  async executeReview(run, signal, reviewEvidence) {
    const reviewAttempts = this.runs.attemptsFor(run.id, "REVIEW").filter((attempt2) => attempt2.role === "review");
    this.budget.assertRoleMayRun(run, "review", reviewAttempts.length, reviewAttempts.reduce((total, attempt2) => total + attempt2.tokens, 0), reviewAttempts.reduce((total, attempt2) => total + attempt2.requests, 0));
    const before = await this.deps.revisions.current();
    ...
    const prepared = await this.prepareGateHandoff(run, "review", before.head, reviewEvidence);
    if ("run" in prepared) return prepared.run;
  ...
  async prepareGateHandoff(run, role, head, cache) {
    try {
      ...
      const key = JSON.stringify([
        ...
        baseline.snapshot.revisionId,
        baseline.snapshot.head,
        run.currentRevisionId,
        head,
        run.mutationEpoch,
        run.configHash,
        configHash(this.deps.config)
      ]);
      ...
      const handoff = this.context.build(role, {
        ...
        evidence: [
          ...
          {
            kind: "review-diff",
            artifact: this.readableArtifact(run, bundle.patch)
          },
          {
            kind: "review-diff-manifest",
            artifact: this.readableArtifact(run, bundle.manifest)
          }
        ]
      });
...
function renderStatus(summary, color = false) {
  ...
  const completion = sealed ? summary.events.findLast((event) => event.type === "RUN_DONE" && event.revision_id === run.currentRevisionId) : void 0;
  let completionNotes = [];
  if (typeof completion?.payload_json === "string") {
    try {
      const payload = JSON.parse(completion.payload_json);
      if (Array.isArray(payload?.notes)) completionNotes = payload.notes.filter((note) => typeof note === "string" && note.trim().length > 0);
    } catch {
    }
  }
...
function statusText(update, frame, activeStage) {
  ...
  const activity = failedDuring ? `${ACTIVITIES[update.run.currentState]} (during ${activeStage === "scout" || activeStage === "archivist" ? ADVISORY_LABELS[activeStage] : displayState(activeStage)})` : ACTIVITIES[update.run.currentState];
  ...
  if (advisory) return `${frame} ${ADVISORY_LABELS[advisory]} \xB7 ${ADVISORY_ACTIVITIES[advisory]}`;
...
function widgetLines(update, frame, activeStage, theme) {
  ...
  const rows = [];
  const addAdvisory = (role) => {
    const status = update.advisory?.[role];
    if (status) rows.push({
      marker: ADVISORY_MARKERS[status],
      label: ADVISORY_LABELS[role],
      activity: `${ADVISORY_ACTIVITIES[role]} (${status})`
    });
  };
  addAdvisory("scout");
  for (const state of STAGES) {
    let marker = interrupted && state === stage ? "!" : stageMarker(state, coreStage);
    if (state === "PLAN" && update.advisory?.scout === "running") marker = "\xB7";
    if (state === "REVIEW" && update.run.currentState === "REVIEW" && (update.advisory?.archivist === "running" || update.advisory?.archivist === "completed" || update.advisory?.archivist === "failed")) marker = "\u2713";
    rows.push({
      marker,
  ...
  return {
    ...
    onProgress: (update) => {
      ...
      if (STAGES.includes(update.run.currentState)) {
        activeStage = ADVISORY_ROLES.find((role) => update.advisory?.[role] === "running") ?? (update.run.currentState === "REVIEW" && (update.advisory?.archivist === "completed" || update.advisory?.archivist === "failed") ? "archivist" : update.run.currentState);
      }
      current = update;
      render();
