...
Architect plans now require a nonempty `smithTasks` array. The implicit single-Smith fallback is removed: Architect explicitly chooses independent work packages or one coupled task. Existing saved plans without `smithTasks` fail schema validation on resume; start a new run rather than editing hashed plan artifacts. Plans that already declare valid tasks need no migration. Sentinel and Inquisitor repair proposals remain optional. Reload the updated extension and use the bundled Architect instructions so the requested output contract and agent policy agree.
