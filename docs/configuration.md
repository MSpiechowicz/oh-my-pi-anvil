...
- The live Forge panel shows enabled Scout before Architect and enabled Archivist after Inquisitor; disabled roles stay hidden. Optional rows track persisted attempts with distinct pending (`·`), running (spinner), completed (`✓`), failed (`!`), and skipped (`–`) markers. Only a completed advisory result is successful; a role never attempted is skipped once its phase passes or the run finishes. Optional failures remain visible without blocking the core workflow. While an optional role runs, it owns the active indicator and the status/working-message fallback rather than Architect or Inquisitor.
...
`implementation.maxParallel` bounds concurrent Smiths (default `4`, range `1..32`; `1` serializes tasks). Architect must supply a nonempty `smithTasks` array in its plan: separate tasks for safe independent work packages, or one explicit task for coupled work. Plan steps are not automatically converted into workers; omitted dispatch is rejected instead of silently choosing one Smith. Each task declares `id`, `objective`, `dependsOn`, `ownedFiles`, `acceptanceCriteria`, and `findingIds`. Ownership is workspace-relative files or directories, not permission to alter unrelated work. Empty ownership means exclusive work. Dependencies must form a DAG; overlapping ownership is serialized. Isolated execution is serialized because independently applying concurrent worktrees is not assumed merge-safe.
