...
export class OmpSubprocessRunner implements AgentRunner {
  ...
  async run<T>(request: AgentRunRequest): Promise<AgentRunResult<T>> { if (!this.compat.execute) return { status: "failed", agentName: request.agentName, usage: { requests: 0 }, resolvedModel: null, resolvedThinkingLevel: null, error: { code: "OMP_EXECUTOR_UNAVAILABLE", message: "No OMP child-agent executor is available in this extension context" } }; return this.compat.execute<T>(request); }
}
