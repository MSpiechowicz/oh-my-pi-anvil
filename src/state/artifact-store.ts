...
export class ArtifactStore {
  ...
  async readText(runId: string, pointer: ArtifactPointer): Promise<string> { const target = containedPath(this.runRoot(runId), pointer.path); const bytes = new Uint8Array(await readFile(target)); if (sha256(bytes) !== pointer.sha256) throw new AnvilError("ARTIFACT_CORRUPT", `Artifact hash mismatch: ${pointer.path}`); return new TextDecoder().decode(bytes); }
