...
export class WorkflowEngine {
  ...
  private async executePlan(run: RunRecord, signal: AbortSignal): Promise<RunRecord> {
    ...
    const result = await this.deps.agents.run<PlanOutput>({ runId: run.id, attemptId: attempt.id, role: "planner", agentName: this.deps.config.agents.planner.agent, assignment: "Produce the strict PlanOutput for this objective.", context: handoff.text, outputSchema: PLAN_OUTPUT_SCHEMA, schemaMode: "strict", cwd: run.workspaceRoot, baseRevisionId: run.currentRevisionId, readOnly: true, signal }); const after = await this.deps.revisions.current(); this.runs.finalizeAttempt(attempt, { ...result, resultRevisionId: after.id });
  ...
  private async executeImplementation(run: RunRecord, signal: AbortSignal): Promise<RunRecord> {
    ...
    const result = await this.deps.agents.run({ runId: run.id, attemptId: attempt.id, role: "implementation", agentName: this.deps.config.agents.implementation.agent, assignment: "Implement the active plan and resolve the referenced open findings.", context: handoff.text, outputSchema: IMPLEMENTATION_OUTPUT_SCHEMA, schemaMode: "strict", cwd: run.workspaceRoot, baseRevisionId: before.id, readOnly: false, isolation: { requested: this.deps.config.implementation.isolation.enabled, apply: true, merge: this.deps.config.implementation.isolation.merge }, signal }); const after = await this.deps.revisions.current(); this.runs.finalizeAttempt(attempt, { ...result, resultRevisionId: after.id });
