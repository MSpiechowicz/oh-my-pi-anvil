...
export class WorkflowEngine {
  ...
  private async executePlan(run: RunRecord, signal: AbortSignal): Promise<RunRecord> {
    ...
    if (after.id !== run.currentRevisionId) throw new AnvilError("READ_ONLY_GATE_MUTATED_WORKSPACE", "Planner mutated the workspace"); if (result.status !== "completed") throw new AnvilError("AGENT_EXECUTION_FAILED", result.error?.message ?? "Planner failed");
  ...
  private async executeImplementation(run: RunRecord, signal: AbortSignal): Promise<RunRecord> {
    ...
    if (result.status !== "completed") throw new AnvilError("AGENT_EXECUTION_FAILED", result.error?.message ?? "Implementation failed"); let output; try { output = requireImplementation(result.structured); } catch (error) { throw asAnvilError(error, "SCHEMA_INVALID"); } if (output.durableLessons?.length) this.lessons.set(run.id, output.durableLessons);
