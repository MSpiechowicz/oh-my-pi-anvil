...
export interface ImplementationOutput { version: 1; status: "completed" | "blocked" | "needs_replan"; summary: string; claimedChangedFiles: string[]; addressedFindingIds: string[]; remainingConcerns: string[]; replanReason?: string; durableLessons?: Array<{ content: string; importance: number }>; }
export type FindingSeverity = "critical" | "high" | "medium" | "low" | "info";
export interface SecurityOutput { version: 1; verdict: "pass" | "findings" | "blocked"; scope: { revisionId: string; reviewedAreas: string[] }; findings: Array<{ severity: FindingSeverity; category: string; title: string; file?: string; lineStart?: number; lineEnd?: number; symbol?: string; description: string; evidence: string; exploitOrImpact: string; fixRequirement: string; confidence: "high" | "medium" | "low"; }>; residualRisks: string[]; blockedReason?: string; }
...
export interface AgentRunRequest<TSchema = unknown> { runId: string; attemptId: string; role: AgentRole; agentName: string; assignment: string; context?: string; outputSchema: TSchema; schemaMode: "strict"; cwd: string; baseRevisionId: string; readOnly: boolean; isolation?: { requested: boolean; apply: boolean; merge: "patch" | "branch" }; signal?: AbortSignal; }
export interface AgentRunResult<T> { status: "completed" | "failed" | "aborted"; structured?: T; agentName: string; resolvedModel?: string; usage: AgentUsage; durationMs: number; rawArtifactRef?: string; stderrArtifactRef?: string; error?: { code: string; message: string }; }
