...
export type AgentRole = "planner" | "implementation" | "security" | "review" | "scout" | "archivist";
...
export interface SmithTask { id: string; objective: string; dependsOn: string[]; ownedFiles: string[]; acceptanceCriteria: string[]; findingIds: string[]; }
export interface SmithDispatchOutput { version: 1; tasks: SmithTask[]; }
export interface PlanOutput { version: 1; summary: string; assumptions: string[]; steps: Array<{ id: string; title: string; objective: string; dependsOn: string[]; fileHints: string[]; symbolHints: string[]; acceptanceCriteria: string[]; risk: "low" | "medium" | "high"; securitySurfaces: string[]; }>; globalAcceptanceCriteria: string[]; requiredChecks: Array<{ id: string; reason: string }>; risks: Array<{ category: string; description: string; mitigation: string }>; replanTriggers: string[]; smithTasks?: SmithTask[]; }
export interface ScoutOutput { version: 1; summary: string; areas: Array<{ path: string; findings: string }>; risks: string[]; recommendations: string[]; }
export interface ArchivistOutput { version: 1; lessons: Array<{ content: string; importance: number }>; }
...
export interface AgentRunRequest<TSchema = unknown> { runId: string; attemptId: string; role: AgentRole; agentName: string; model?: string | null; thinkingLevel?: string | null; effort?: string | null; assignment: string; context?: string; outputSchema: TSchema; schemaMode: "strict"; cwd: string; baseRevisionId: string; readOnly: boolean; isolation?: { requested: boolean; apply: boolean; merge: "patch" | "branch" }; signal?: AbortSignal; }
...
export interface CheckDefinition { id: string; command: string[]; cwd?: string | null; env?: Record<string, string> | null; required: boolean; timeoutMs: number | null; }
...
export interface WorkflowConfig { version: 1; workflow: { name: string; }; agents: Record<AgentRole, { agent: string; model?: string | null; thinkingLevel?: string | null; effort?: string | null }>; checks: CheckDefinition[]; checksFailFast: boolean; security: { failOn: FindingSeverity[]; policyVersion: number; }; review: { blockOn: Array<"blocking" | "major" | "minor">; policyVersion: number; }; implementation: { maxParallel?: number; isolation: { enabled: boolean; merge: "patch" | "branch"; }; }; planning: { maxGenerations: number | null; }; clarification: { mode: "auto" | "always" | "off"; maxRounds: number; }; scouting: { enabled: boolean; }; budgets: { maxTotalTokens?: number | null; maxTotalRequests?: number | null; maxTransitions?: number | null; maxWallClockMs?: number | null; perRole: Partial<Record<AgentRole, { maxTokens?: number | null; maxAttempts?: number | null; maxRequests?: number | null }>>; }; context: { maxInlineChars: number; maxMemoryItems: number; maxMemoryChars: number; maxFindingSummaryChars: number; maxChangedFiles: number; }; memory: { enabled: boolean; retainOnSuccess: boolean; archivist: boolean; maxRetainedLessons: number; }; persistence: { root: string; keepAgentRawArtifacts: boolean; keepCommandLogs: boolean; persistRenderedPrompts: boolean; }; safety: { oneMutatingRunPerWorkspace: boolean; securityMustBeReadOnly: boolean; reviewerMustBeReadOnly: boolean; refusePathEscapeFromWorkspace: boolean; }; }
