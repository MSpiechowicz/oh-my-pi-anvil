...
export const TERMINAL_STATES = new Set<WorkflowState>(["DONE", "BLOCKED", "FAILED", "CANCELLED"]);
...
export function assertActive(run: RunRecord): void { if (isTerminal(run.currentState)) throw new Error(`Run ${run.id} is already ${run.currentState}`); }
