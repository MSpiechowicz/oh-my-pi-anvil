  ...
  async handle(raw: string, context: CommandContext): Promise<string> {
    ...
    try {
      ...
      switch (command) {
        ...
        case "resume": if (!rest[0]) throw new AnvilError("CONFIG_INVALID", "Usage: /orchestrate resume <run-id>"); return renderStatus(await engine.resume(rest[0]));
        case "cancel": if (!rest[0]) throw new AnvilError("CONFIG_INVALID", "Usage: /orchestrate cancel <run-id>"); await engine.cancel(rest[0]); return renderStatus(engine.status(rest[0]));
        ...
        default: throw new AnvilError("CONFIG_INVALID", `Unknown /orchestrate command: ${command}`);
      }
